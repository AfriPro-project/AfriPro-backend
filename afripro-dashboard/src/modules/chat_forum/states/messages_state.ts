...
export const fetchmessages=async()=>{
    let response = await get('/messages/admin/group');
...
export const fetchChatRoomMessages=async(id:string)=>{
    ...
    messagesState.staticChatRoomMessages.set(response['messages']);
    messagesState.chatRoomMessages.set(response['messages']);
}
